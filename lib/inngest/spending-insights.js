    ...
    for (const user of users) {
      ...
      const expenseData = JSON.stringify({
        expenses: expenses.map(expense => ({
          ...expense,
          amount: `₹${expense.amount.toFixed(2)}` // Format as rupees
        })),
        totalSpent: `₹${expenses.reduce((sum, e) => sum + e.amount, 0).toFixed(2)}`,
        categories: Object.entries(expenses.reduce((cats, e) => {
          cats[e.category ?? "uncategorised"] =
            (cats[e.category] ?? 0) + e.amount;
          return cats;
        }, {})).reduce((formatted, [category, amount]) => {
          formatted[category] = `₹${amount.toFixed(2)}`;
          return formatted;
        }, {}),
      });
...
IMPORTANT: All amounts should be displayed in Indian Rupees (₹) format, not dollars ($).

User spending data:
${expenseData}

Provide your analysis in these sections:
1. Monthly Overview (show total spent as ₹X,XXX format)
2. Top Spending Categories (use ₹ symbol for all amounts)
3. Unusual Spending Patterns (if any, with ₹ amounts)
4. Saving Opportunities (mention savings in ₹)
5. Recommendations for Next Month (use ₹ for budget suggestions)

Remember: Use ₹ (Indian Rupee) symbol for ALL monetary values, not $ (dollar) symbol.
      `.trim();
